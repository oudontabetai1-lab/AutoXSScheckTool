...
QR画像を選ぶと TOTP 登録情報として確認し、発行者・アカウント・桁数・周期を表示します。成功すると Base32 シークレット欄（マスク表示）と生成条件に自動反映されます。読取用画像は保存せず、設定 export にシークレットを含めません。壊れた画像、TOTP 以外の QR、デコーダ未導入は理由を表示します。確認中・失敗後は画像の再選択、または URI / Base32 / サーバ側パスの入力が必要です。
