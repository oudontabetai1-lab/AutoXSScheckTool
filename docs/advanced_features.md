...
1. **設定 → 「攻撃シナリオ」タブ**: スキャン開始前にシナリオを作成。保存したシナリオは
   スキャン設定とともに送信される。
2. **巡回レビュー画面（任意）**: `interactive_crawl_review` を有効にすると、巡回完了後に
   発見した画面・フォーム・入力欄の一覧（パレット）を見ながらシナリオを組める。クリックで
   「移動」「入力」「フォーム一式（移動＋各入力＋送信）」ステップをワンタッチ追加可能。
   作成したシナリオは「検査続行」時に取り込まれ、検査前に実行される。
