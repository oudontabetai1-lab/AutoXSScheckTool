...
| `url_normalize.py` / `injection_point.py` | `InjectionPoint.stable_key_parts` は resume キーに `normalize_url_for_key` で既知の揮発クエリだけを除いた URL を使う（実攻撃 URL は不変、未知キーは保持） |
