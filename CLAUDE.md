...
- **Agent の誠実性（0007 D8・Agent 層）**: 初期化/実行のハードエラーを「0 findings の正常完了」に見せない。`AgentBrowserScanner.run` は `result.error` があれば FAILED 表示、`agent_engine`/`main.run_agent` は成功レポート/完了イベントを出さず result を返し、CLI は `main._agent_exit_code(result)`（result.error 非空→1）で**非0 exit**する。設定ディレクトリ書込み不可（`check_agent_config_directory`）は起動前に**警告（案内）**するが**中断はしない**（`os.access` は sandbox 等で誤判定しうる／browser-use が XDG・遅延生成で回避しうるため。実失敗は上記 result.error 経由で誠実に表面化する）。案内は `XDG_CONFIG_HOME` を書込み可へ、を含む。
