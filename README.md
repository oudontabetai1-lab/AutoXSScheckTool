...
ダッシュボードの「手動巡回」では、サーバ上の headless Chromium を遠隔操作できます。
リンクが新しいタブを開いた場合も表示・入力対象が自動で切り替わり、閉じると直前の残存タブへ戻ります。
...
手動で選ぶには「手動巡回」で遠隔ブラウザを起動し、ログイン操作で OTP 画面へ進んで「OTP欄を指定」を押し、入力欄そのものをクリックします。選択した selector が認証設定へ反映されます。TOTP URI・Base32・QR のいずれかを設定してから遠隔ブラウザを起動した場合は、欄の指定と同時に現在コードを入力します。期限切れ時は「TOTPを再入力」で同じ selector へ新しいコードを入力できます。生成コードは API レスポンスやイベント履歴には含めません。複数の欄に1桁ずつ分割された OTP、iframe 内の欄など、自動で一意に特定できない画面では手動ログインで Cookie を取得してください。
