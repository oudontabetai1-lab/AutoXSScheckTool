...
通常層の確実性を保ちながら SPA の非同期 API を取りこぼさないため、Angular / React / Next.js / Vue / Nuxt の固有マーカー（本番ビルドの `self.__next_f`・`__NUXT_DATA__` や、空のマウント要素＋ハッシュ付きバンドルの本番シェルを含む）を初回ページで検出すると SPA クロールを自動有効化します（既定 ON）。マウントに内容がある静的サイトなど一般的なページ構成では有効化せず、不要なクリック探索を避けます。自動判定を使わない場合は `--no-auto-spa`、常に有効化する場合は `--spa-crawl` を指定します。
